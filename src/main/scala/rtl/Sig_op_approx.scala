...
class Sig_op_approx(nBits: Int, eBits: Int) extends Module {
  ...
  override def desiredName = s"Sig_op_approx${nBits}_${eBits}"
  val decA = RawFloatFromPosit(io.A, nBits, eBits)
  val decB = RawFloatFromPosit(io.B, nBits, eBits)
//printf("Print during simulation: Input is %d\n", decA.scale)
 val scale_diff  = decA.scale-decB.scale
  val aGTb        = ~scale_diff.head(1).asBool
  ...
  val sdiff = Mux(aGTb,scale_diff,-scale_diff)
  io.AisNar := decA.isNaR
  io.BisNar := decB.isNaR  
  io.AisZero := decA.isZero
  io.BisZero := decB.isZero
  val sSig   = Cat(io.smallerSign, ~(io.smallerSign | smallerZero), smallerFrac,0.U(3.W)).shiftRightArithSticky(sdiff.asUInt)
